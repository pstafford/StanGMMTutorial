...
Similarly, he mixture model can be used to combine different GMPEs into a single model as in [@Haendel2014].
...
Mixture models are in general difficult to estimate via MCMC, since they are not identifiable (one could swap the indices of $w$ and $\phi_{SS}$ and end up with the same lkelihood), which means that different chains can have swapped estimated parameters, even though they have converged.
Sometimes, his can happen within a chain.
...
## Custom Data Likelihoods

In the previous section, we have replaced the normal distribution with a mixture of two normal distributions as the likelihood of the record level (for robust regression, we had used the Student-_t_ distribution).
One could also use a different distribution; for example, [@Bullock2019] remarked that a loglogistic distribution might be more ``conservative'' than the lognormal distribution in PSHA, which again raises the question whether coefficients estimated would be different.

To implement a model with a loglogistic likelihood at the record level, we need to specify the density of the distribution first, which is done in the optional `functions` block.
Then, the log-probability needs to be incremented by the value of the density.

The density of the loglogistic distribution is
$$
p(y) = \frac{(\beta/\alpha) (y/\alpha)^{\beta-1}}{(1 + (y/\alpha)^\beta)^2}
$$
where the median is $\alpha$ and the standard deviation is $1/\beta$.
This function is implemented before the main Stan program, and then used in the model block to increment the log-probability.
Note that the loglogistic distribution is a distribution for the absolute GM values, not the logarithmic ones.

```{stan, output.var="gmm_loglogistic"}
functions {
  real loglogistic(real x, real alpha, real beta) {
...
model {
  ...
  for(i in 1:N) {
    ...
    target += log(loglogistic(Y_exp[i], exp(mu[i]), 1/phiSS));
  }
}
```
...
So far, we showed different models that can be fit using Stan.
It is possible to use Stan to make predictions as well.
This can be done in an optional `generated quantities` block.
In this block, the current sample of each parameter is used, withut any effect on the posterior.
There are two ways one can make use of the `generated quantities` block -- during fitting, and for pure prediction.

Below, we show one can calculate median predictions for a simple model (not a GMM).
Evry variable that is defined in the `generated quantities` block shows up in the samples like a parameter, so this generates a list of median predictions for all samples of the posterior distribution.
The generated `quantities block` can also be used to calculate residuals or log-likelihood values (which can be used with package `loo` for model comparison).

``` {stan, output.var = "median_prediction"}
data {
  int<lower=1> N;
  vector[N] X;
  vector[N] Y;
...
It is easy to change the above code to also calculate predictions for new `X` values - one just declares a new variable `X_pred` as data, together with its length `N_pred`, and changes the variable used in the `generated quantities` block.
It would be inefficient to fit the same model every time one wants to calculate new predictions.
Instead, one can write a program where the variables are given as data, and the model block is empty, as shown below.

``` {stan, output.var = "prediction_gq"}
data {
  int<lower=1> N;
...
generated quantities {
  matrix[N_samples,N] mu;
  
  for(i in 1:N_samples)
    mu[i] = (alpha + beta * X_pred)';
}
```

Here, the input are a bunch of samples for the parameters (e.g. from the posterior distribution).
The predictions are stored in a matrix.
Since the $i$th row of matrix `mu` is a `row_vector`, we need to transpose the vectorized statement `alpha + beta * X_pred`.
Alternatively, we could have also used the declaration `row_vector[N] X_pred`.

Since the Stan program above contains an empty model block, it needs to be run with `algorithm = "Fixed_param"`.

```{r eval=FALSE}

pred <- stan('predict.stan',
             data = data_pred,
             chains = 1,
             iter = 1,
...
Alternatively, one can of course also write a function directly in **R** (or **python** and so on) that calculates predictions for any given parametere values and predictor variables.
